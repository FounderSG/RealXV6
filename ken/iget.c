...
loop:
    ...
    ip1 = (uint)bp->b_addr + 32*lrem(ino+31, 16);
    ip2 = &p->i_mode;
...
void iupdat(struct inode *p, int *tm)
{
    ...
    if((rp->i_flag&(IUPD|IACC)) != 0) {
        ...
        ip1 = (uint)bp->b_addr + 32*lrem(i, 16);
        ip2 = &rp->i_mode;
...
void itrunc(struct inode *aip)
{
    ...
    if(*ip) {
        if((rp->i_mode&ILARG) != 0) {
            ...
            for(cp = (uint)bp->b_addr+512; cp >= bp->b_addr; cp--)
            if(*cp) {
                if(ip == &rp->i_addr[7]) {
                    dp = bread(rp->i_dev, *cp);
                    for(ep = (uint)dp->b_addr+512; ep >= dp->b_addr; ep--)
                    if(*ep)
                        free(rp->i_dev, *ep);
                    brelse(dp);
                }
