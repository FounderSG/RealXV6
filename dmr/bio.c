...
void physio(void (*strat)(struct buf *), struct buf *abp, int dev, int rw)
{
     ...
     * reported to the caller as "not transferred".
     */
    nsect = u.u_count >> 9;
    if(nsect == 0)
        return;
    ...
    pp = u.u_procp;
    /*
     * Locate the user buffer's ABSOLUTE (identity-mapped) address.  V6
     * simulated the PDP-11 segmentation registers here; this port has no MMU
     * and the process image is one flat block at p_addr, so a user address is
     * just an offset into it.  It must be resolved to an absolute address
     * rather than reached through the running process, because the transfer
     * finishes at interrupt time when another process may be current -- the
     * same reason swap() addresses core by page number.
     */
    page = pp->p_addr + (base >> 12);
    off = base & (PAGESIZ-1);
