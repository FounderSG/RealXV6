#
/*
 * Unix V6 mkfs from source/s2/mkfs.c for modern host compilation
 *
...
f2:
	...
	swap = getnum();
	printf("%s: %d file blocks, %d inode blocks, %d swap\n",
		fsys, filsys.s_fsize, filsys.s_isize, swap);
...
f3:
	...
	for(n=0; n!=filsys.s_isize; n++)
		wtfs(n+2, buf);
	n = filsys.s_fsize+swap-1;      /* extend image to filesys + swap size */
	wtfs(n, buf);
	cfile((void *)0);
...
struct inode *par;
{
	...
	in.i_mode |= gmode(string[1], "u", ISUID, 0, 0);
	in.i_mode |= gmode(string[2], "g", ISGID, 0, 0);
	for(i=3; i<6; i++) {
